  ...
  public class MainPlayer : Player
  {
    ...
    public float getCooldown()
    {
      ...
      if (whands != null)
          ...
          return ((float)TimeSpan.FromSeconds(Resources.MeleeList[0].Cooldown).TotalMilliseconds);
    }
    ...
    void setSprinting(DeviceState state)
    {
      if (!_sprinting && Options.Config.Bindings[GameAction.Sprint].IsControlDown(state)) _sprinting = true;
    }

    //Code to move the player
    void updateMove(GameTime gameTime, DeviceState state, Level level)
    {
      ...
      else if (Options.Config.Bindings[GameAction.StrafeRight].IsControlDown(state))
      {
        if (Options.Config.Bindings[GameAction.Forward].IsControlDown(state) || Options.Config.Bindings[GameAction.Backward].IsControlDown(state))
          speed *= 0.5f;
        movement += new Vector2((float)Math.Cos(orientation) * speed, (float)-Math.Sin(orientation) * speed);
      ...
      if (Options.Config.Bindings[GameAction.Backward].IsControlDown(state))
          movement += new Vector2((float)Math.Sin(orientation) * speed, (float)Math.Cos(orientation) * speed);
    ...
    public void updateHands(GameTime gameTime, DeviceState state, DeviceState prevState)
    {
      ...
      if (state.mouse.LeftButton == ButtonState.Pressed && prevState.mouse.LeftButton == ButtonState.Released && !cooldown)
      {
				useHands();
        cooldown = true;
      ...
      if (state.mouse.RightButton == ButtonState.Pressed)
        _aimlock = true;
      else if (state.mouse.RightButton == ButtonState.Released)
        _aimlock = false;
    }
    ...
    public bool checkState(PlayerStates state)
    {
      for (int i = 0; i < _states.Count; i++)
        if (_states[i] == state) return (true);
      return (false);
    }
    ...
    public void updateNetwork(GameTime gameTime)
    {
      ...
      //Base update call
    public void Update(GameTime gameTime, Level level, DeviceState state, DeviceState prevState, Camera cam)
    {
      if (!alive) return;
      if (!inventoryOpen) updateAimAngle(cam, state, prevState);
      updateMove(gameTime, state, level);
    ...
    {
        spriteBatch.Draw(Resources.Players, pos, _skinRect, Color.White, -orientation, new Vector2(16, 8), 1.0f, SpriteEffects.None, 0.3f);
    }
  }
}