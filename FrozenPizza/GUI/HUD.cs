  ...
  public class HUD
  {
    ...
    public Rectangle calcHeadsUpRect(int maxvalue, int value)
    {
      ...
      return (rect);
		...
		private void DrawInventory(SpriteBatch spriteBatch, MainPlayer mainPlayer)
		{
			//Background Panel
			DrawHudPanel(spriteBatch, _inventoryPanel, Color.Gray, 0.9f);
		}

		private void DrawHudPanel(SpriteBatch spriteBatch, Rectangle panel, Color color, float opacity)
		{
			spriteBatch.Draw(_colorRect, panel, null, color * opacity, 0f, Vector2.Zero, SpriteEffects.None, 0.2f);
		}

		public void drawHandsPanel(SpriteBatch spriteBatch, MainPlayer mainPlayer)
		{
			DrawHudPanel(spriteBatch, _handsPanel, Color.LightGray, 0.5f);
			if (mainPlayer.cooldown)
			DrawHudPanel(spriteBatch, _cooldownBar, Color.White, 0.75f);
      if (mainPlayer.hands == null)
      {
          spriteBatch.DrawString(_font, "Hands", _handsPanel.Location.ToVector2(), Color.White);
          spriteBatch.Draw(Resources.Tilesets[(int)ItemType.Melee], new Vector2(_handsPanel.X + _handsPanel.Width / 2, _handsPanel.Y + _handsPanel.Height / 2), new Rectangle(0, 0, 32, 32), Color.White, 0f, new Vector2(16, 16), 1.0f, SpriteEffects.None, 0f);
      }
      else
      {
        spriteBatch.DrawString(_font, mainPlayer.hands.name, _handsPanel.Location.ToVector2(), Color.White);
        spriteBatch.Draw(Resources.Tilesets[(int)mainPlayer.hands.type], new Vector2(_handsPanel.X + _handsPanel.Width / 2, _handsPanel.Y + _handsPanel.Height / 2), mainPlayer.hands.skinRect, Color.White, 0f, new Vector2(16, 16), 1.0f, SpriteEffects.None, 0f);
		...
		public void Draw(SpriteBatch spriteBatch,MainPlayer mainPlayer, Camera cam)
    {
			_hudEntRect.X = 0;
      //Health
      ...
      spriteBatch.Draw(_hudEntities, _statsPos + getHeadsUpHeight(mainPlayer.hp.max, mainPlayer.hp.get(), false), calcHeadsUpRect(mainPlayer.hp.max, mainPlayer.hp.get()), Color.White, 0, Vector2.Zero, 1.0f, SpriteEffects.None, 0);
			...
			//Hands Panel
			drawHandsPanel(spriteBatch, mainPlayer);
			//Inventory
            if (mainPlayer.inventoryOpen)
				DrawInventory(spriteBatch, mainPlayer);
			//Cursor & AimLines
